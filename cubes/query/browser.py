...
class AggregationBrowser(object):
    ...
    def prepare_aggregates(self, aggregates=None, measures=None):
        ...
        if aggregates and measures:
            raise ArgumentError(
                "Only aggregates or measures can be specified, not both"
            )
        if aggregates:
            try:
                ...
                raise NoSuchAttributeError(
                    "No measure aggregate '%s' in cube '%s'" % (str(e), str(self.cube))
                )
        elif measures:
            aggregates = []
        ...
        for agg in aggregates:
            if (
                ...
                try:
                    ...
                    raise NoSuchAttributeError(
                        "Cube '%s' has no measure aggregate "
                        "'%s' for '%s'" % (self.cube.name, agg.measure, agg.name)
                    )
...
def levels_from_drilldown(cell, drilldown):
    ...
    * (``dimension@hierarchy:level``) list of tuples in form (`dimension`,
    ...
    if isinstance(drilldown, dict):
        logger = get_logger()
        logger.warn(
            "drilldown as dictionary is depreciated. Use a list of: "
    ...
    for obj in drilldown:
        if isinstance(obj, compat.string_type):
            ...
            raise ArgumentError(
                "Drilldown item should be either a string "
